...
// 職責：渲染「公司詳細資料頁」的所有UI元件 (Bento Grid: 視覺一致性與原地編輯版 - Event Delegation Refactor)
...
function _renderViewMode(info) {
    ...
    // 【修改】移除 onclick，改用 data-action="edit-mode"
    return `
        <div class="company-info-wrapper" id="company-info-card-container">
...
function _renderEditMode(info) {
    ...
    // 【修改】移除 onsubmit, onclick。 Form 提交事件由 events.js 統一處理。
    return `
...
function renderCompanyContactsTable(contacts) {
    ...
    contacts.forEach(contact => {
        // 【修改】將 contact 物件轉為 JSON 字串，安全地放入 data-contact
